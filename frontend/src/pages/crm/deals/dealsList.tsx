...
const DealsList = () => {
  ...
  const paymentsActions = getActionsForPage('deals');
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: any) => (
        <span
          className={`badge badge-tag px-1.5 py-0.5 rounded text-[12px]  ${
            text === "Collab"
              ? "border-b bg-success-100 text-success border-success"
              : text === "Rated"
              ? "border-b bg-warning-100 text-warning border-warning"
              : text === "Promotion"
              ? "border-b bg-info-100 text-info border-info"
              : "border-b bg-danger-100 text-danger border-danger"
          } `}
        >
          {text}
        </span>
