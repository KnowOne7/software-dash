...
import LostReasonModal from "./modal/lostReasonModal";

const LostReason = () => {
  const data = LostReasonListData;
    // Get actions for payments page
    const lostReasonActions = getActionsForPage('lostReason');
    const actionColumn = useActionColumn(lostReasonActions);
  const columns = [
    {
      title: "Title",
      dataIndex: "Title",
